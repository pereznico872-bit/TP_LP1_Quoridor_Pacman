/* Libreria grafica. Permite abrir ventana, dibujar y leer teclado/mouse. */
#include "raylib.h"

/* Librerias estandar de C */
#include <stdio.h>    /* FILE, fopen, fprintf, fscanf, fclose */
#include <stdlib.h>   /* rand, srand */
#include <stdbool.h>  /* bool, true, false */
#include <string.h>   /* strcpy, strncpy, strcmp, memset */
#include <time.h>     /* time, para inicializar numeros aleatorios */

/* Constantes generales del juego.
   Se usan con #define para no escribir numeros sueltos por todo el codigo. */
#define MAX_SIZE 15          /* tamaño maximo permitido del tablero */
#define MIN_SIZE 5           /* tamaño minimo permitido del tablero */
#define NUM_GHOSTS 4         /* cantidad fija de fantasmas */
#define NUM_PELLETS 4        /* cantidad fija de pac-bolas */
#define MAX_TEMP_WALLS 128   /* maximo de muros temporales que puede guardar el juego */

/* Medidas para dibujar el tablero en pantalla. */
#define CELL 48              /* tamaño en pixeles de cada casilla */
#define BOARD_X 40           /* posicion X donde empieza el tablero */
#define BOARD_Y 60           /* posicion Y donde empieza el tablero */
#define SIDE_X 560           /* posicion X del panel de informacion */

/* Dueño de un muro temporal o ganador de la partida. */
#define OWNER_PAC 0
...
/* Pos guarda una coordenada logica del tablero.
   r = row/fila, c = column/columna.
   Ejemplo: Pos{4, 4} significa fila 4, columna 4. */
typedef struct {
    int r;  /* fila */
    int c;  /* columna */
} Pos;

/* Map guarda todo lo que pertenece al mapa:
   tamaño, posiciones iniciales y muros permanentes. */
typedef struct {
    int rows;                          /* cantidad de filas del tablero */
    int cols;                          /* cantidad de columnas del tablero */
    Pos pacStart;                      /* posicion inicial de Pac-Man */
    Pos ghostStart[NUM_GHOSTS];        /* posiciones iniciales de los 4 fantasmas */
    Pos pelletStart[NUM_PELLETS];      /* posiciones de las 4 pac-bolas */

    /* Los muros no estan dentro de una casilla, sino ENTRE casillas.
       wallRight[r][c] bloquea entre (r,c) y (r,c+1).
       wallDown[r][c] bloquea entre (r,c) y (r+1,c). */
    bool wallRight[MAX_SIZE][MAX_SIZE];
    bool wallDown[MAX_SIZE][MAX_SIZE];
} Map;

/* Ghost guarda los datos de un fantasma. */
typedef struct {
    Pos start;          /* posicion inicial del fantasma */
    Pos pos;            /* posicion actual del fantasma */
    bool enabled;       /* si esta habilitado en la configuracion */
    bool alive;         /* si esta vivo/aparece en el tablero */
    int difficulty;     /* 1 facil, 2 normal, 3 dificil */
    const char *name;   /* nombre: Blinky, Inky, Pinky o Clyde */
    Color color;        /* color con el que se dibuja */
} Ghost;

/* TempWall representa un muro temporal colocado durante la partida. */
typedef struct {
    int r;        /* fila base del muro */
    int c;        /* columna base del muro */
    int dir;      /* WALL_RIGHT o WALL_DOWN */
    int owner;    /* OWNER_PAC u OWNER_GHOST */
    int life;     /* turnos restantes antes de desaparecer */
    bool active;  /* true si el muro sigue en el tablero */
} TempWall;

/* Config guarda las opciones elegidas en el menu antes de empezar. */
typedef struct {
    GameMode mode;                         /* IA vs Jugador o Jugador vs Jugador */
    bool ghostEnabled[NUM_GHOSTS];         /* si cada fantasma aparece o no */
    int ghostDifficulty[NUM_GHOSTS];       /* dificultad individual de cada fantasma */
    int pacWallsStart;                     /* muros iniciales de Pac-Man */
    int ghostWallsStart;                   /* muros iniciales del equipo fantasma */
    int wallLife;                          /* vida de cada muro temporal */
    int mapIndex;                          /* 0,1,2 predefinidos; 3 custom_map.txt */
} Config;
/* Game es el estado completo de una partida.
   Es como la memoria principal del juego mientras se esta jugando. */
typedef struct {
    Map map;                                /* mapa cargado para esta partida */
    Config config;                          /* configuracion usada */
    Pos pac;                                /* posicion actual de Pac-Man */
    Ghost ghosts[NUM_GHOSTS];               /* datos de los 4 fantasmas */
    bool pelletAlive[NUM_PELLETS];          /* indica si cada pac-bola sigue en el tablero */
    int lives;                              /* vidas restantes de Pac-Man */
    int pelletsEaten;                       /* cantidad de pac-bolas comidas */
    int pacWallsHand;                       /* muros disponibles de Pac-Man */
    int ghostWallsHand;                     /* muros disponibles de fantasmas */
    TempWall tempWalls[MAX_TEMP_WALLS];     /* arreglo de muros temporales */
    int tempWallCount;                      /* cantidad de muros temporales creados */
    TurnType turn;                          /* turno actual: Pac-Man o fantasmas */
    int currentGhost;                       /* fantasma que esta jugando ahora */
    int pacActions;                         /* acciones restantes de Pac-Man */
    int globalTurn;                         /* contador de turnos globales */
    bool gameOver;                          /* true si termino la partida */
    int winner;                             /* 0 Pac-Man, 1 fantasmas */
    char message[160];                      /* mensaje que se muestra en pantalla */
    float messageTime;                      /* tiempo restante para mostrar el mensaje */
    float aiTimer;                          /* pausa para que la IA no juegue instantaneamente */
} Game;
...
/* main es la entrada principal del programa.
   Aca se crea la ventana, se inicializan datos y se ejecuta el ciclo principal. */
int main(void)
{
    ...
    /* CICLO PRINCIPAL DEL JUEGO.
       Se repite muchas veces por segundo mientras la ventana siga abierta. */
    while (!WindowShouldClose()) {

        /* 1) ACTUALIZAR:
           Segun la pantalla actual, se lee teclado/mouse y se cambia la logica. */
        if (screen == SCREEN_CONFIG) UpdateConfig();           /* menu */
        else if (screen == SCREEN_GAME) UpdateGame(&game);     /* partida */
        else if (screen == SCREEN_EDITOR) UpdateEditor(&editor);/* editor */
        else if (screen == SCREEN_END) {                       /* pantalla final */
            if (IsKeyPressed(KEY_R)) {
                InitGame(&game, &config); /* reinicia usando la configuracion actual */
                screen = SCREEN_GAME;     /* vuelve al juego */
            }
            if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_ESCAPE)) {
                screen = SCREEN_CONFIG;   /* vuelve al menu */
            }
        }

        /* 2) DIBUJAR:
           Se limpia la pantalla y se dibuja lo que corresponda. */
        BeginDrawing();
        ClearBackground((Color){25, 25, 35, 255});

        if (screen == SCREEN_CONFIG) DrawConfig();          /* dibuja menu */
        else if (screen == SCREEN_GAME) DrawGame(&game);    /* dibuja partida */
        else if (screen == SCREEN_EDITOR) DrawEditor(&editor);/* dibuja editor */
        else if (screen == SCREEN_END) DrawEndScreen(&game);/* dibuja final */
...
/* Carga valores iniciales por defecto para el menu de configuracion. */
static void InitDefaultConfig(Config *cfg)
{
    int i;
    cfg->mode = MODE_AI;
...
/* Actualiza el menu de configuracion:
   lee flechas, cambia opciones, entra al editor o inicia la partida. */
static void UpdateConfig(void)
{
...
/* Dibuja el menu de configuracion:
   opciones, mapa seleccionado, dificultad e imagen personalizada. */
static void DrawConfig(void)
{
...
/* Limpia/crea un mapa con cierto tamaño.
   Tambien pone posiciones iniciales simples y borra todos los muros. */
static void ClearMap(Map *m, int rows, int cols)
{
    int r, c, i;
...
/* Carga uno de los 3 mapas predefinidos.
   index 0 = mapa 1, index 1 = mapa 2, index 2 = mapa 3. */
static void LoadPredefinedMap(Map *m, int index)
{
    int r, c;
    if (index == 0) {
...
/* Guarda un mapa en un archivo de texto.
   Usa fprintf para escribir filas, columnas, posiciones y muros. */
static bool SaveMapToFile(const Map *m, const char *filename)
{
...
/* Carga un mapa desde archivo de texto.
   Lee etiquetas como ROWS, COLS, PAC, GHOST, PELLET y WALL. */
static bool LoadMapFromFile(Map *m, const char *filename)
{
...
/* Inicializa una partida nueva usando la configuracion elegida.
   Carga mapa, coloca Pac-Man/fantasmas, vidas, pac-bolas, turnos y muros. */
static void InitGame(Game *g, const Config *cfg)
{
...
/* Actualiza la logica de la partida:
   mueve Pac-Man, mueve fantasmas, coloca muros, cambia turnos y revisa fin. */
static void UpdateGame(Game *g)
{
    if (IsKeyPressed(KEY_R)) {
        InitGame(g, &config);
...
/* Intenta mover a Pac-Man en una direccion.
   Primero verifica si tiene acciones y si no hay muro/borde bloqueando. */
static void MovePac(Game *g, Direction d)
{
    Pos np;
...
/* Mueve un fantasma.
   Si ve a Pac-Man en linea recta sin muros, se activa modo frenetico y mueve 2 pasos. */
static void MoveGhost(Game *g, int ghostIndex, Direction d)
{
    int steps = 1;
    int s;
...
/* Coloca un muro temporal.
   Verifica limites, que no exista otro muro ahi y que el jugador tenga muros en mano. */
static bool PlaceTempWall(Game *g, int owner, int r, int c, int dir)
{
    int i;
    if (dir == WALL_RIGHT) {
...
/* Termina el turno de Pac-Man y pasa al turno de fantasmas. */
static void EndActorTurn(Game *g)
{
    TickTempWalls(g);
    g->globalTurn++;
...
/* Baja la vida de todos los muros temporales activos.
   Si un muro llega a 0, desaparece y vuelve a la mano de su dueño. */
static void TickTempWalls(Game *g)
{
    int i;
    for (i = 0; i < g->tempWallCount; i++) {
...
/* Cuando un fantasma atrapa a Pac-Man, se reinician las posiciones.
   Las pac-bolas comidas no vuelven y los muros temporales quedan. */
static void ResetPositionsAfterCapture(Game *g)
{
    int i;
    g->pac = g->map.pacStart;
...
/* Revisa si Pac-Man esta en una pac-bola.
   Si la come, desaparece y Pac-Man gana una accion extra. */
static void CheckPacPellet(Game *g)
{
    int i;
...
/* Revisa si Pac-Man piso un fantasma.
   En este proyecto, si Pac-Man pisa un fantasma, lo come y lo saca del tablero. */
static void CheckPacGhostCollisionAfterPacMove(Game *g)
{
    int i;
    for (i = 0; i < NUM_GHOSTS; i++) {
...
/* Revisa si un fantasma llego a la casilla de Pac-Man.
   Si ocurre, Pac-Man pierde una vida. */
static void CheckGhostCatchesPac(Game *g, int ghostIndex)
{
...
/* Devuelve true si una fila/columna esta dentro del tablero. */
static bool IsInside(const Map *m, int r, int c)
{
    return r >= 0 && r < m->rows && c >= 0 && c < m->cols;
}

/* Calcula la casilla resultante al dar un paso en una direccion. */
static Pos Step(Pos p, Direction d)
{
    if (d == DIR_UP) p.r--;
...
/* Revisa si hay un muro entre dos casillas.
   Considera muros permanentes y muros temporales. */
static bool IsBlocked(const Game *g, int r1, int c1, int r2, int c2)
{
    int wr = -1, wc = -1, wd = -1;
    int i;
...
/* Dice si una ficha puede moverse desde p hacia d.
   Usa Step para calcular destino y IsBlocked para revisar muros. */
static bool CanMove(const Game *g, Pos p, Direction d)
{
    Pos np = Step(p, d);
...
/* Distancia Manhattan:
   diferencia de filas + diferencia de columnas.
   Sirve para que la IA mida que movimiento acerca mas al fantasma a Pac-Man. */
static int Manhattan(Pos a, Pos b)
{
    int dr = a.r - b.r;
    int dc = a.c - b.c;
...
/* Revisa si un fantasma ve a Pac-Man en la misma fila o columna,
   sin muros en el medio. Si lo ve, puede moverse 2 pasos. */
static bool GhostSeesPac(const Game *g, int ghostIndex)
{
...
/* IA del fantasma.
   Prueba las direcciones posibles y elige segun dificultad:
   facil = mas aleatorio, normal = mezcla, dificil = persigue mejor. */
static Direction ChooseGhostDirection(Game *g, int ghostIndex)
{
...
/* Guarda un mensaje temporal para mostrar en pantalla. */
static void SetMessage(Game *g, const char *text)
{
...
/* Dibuja la grilla base del tablero. */
static void DrawBoardBase(const Map *m)
{
    int r, c;
    for (r = 0; r < m->rows; r++) {
...
/* Dibuja muros permanentes y temporales.
   Permanentes: azul. Temporales: amarillo o rojo con numero de vida. */
static void DrawWalls(const Map *m, const TempWall *tw, int count)
{
    int r, c, i;
    for (r = 0; r < m->rows; r++) {
...
/* Dibuja toda la partida: tablero, muros, pac-bolas, Pac-Man, fantasmas y panel. */
static void DrawGame(const Game *g)
{
    int i;
    DrawBoardBase(&g->map);
...
/* Dibuja el panel derecho con vidas, turnos, acciones, muros e instrucciones. */
static void DrawHud(const Game *g)
{
    int x = SIDE_X;
    int y = 45;
    ...
    /* Cuadro de mensajes temporales.
       Se muestra solo mientras messageTime sea mayor que 0. */
    if (g->messageTime > 0) {
...
/* Dibuja la pantalla final con ganador, pac-bolas comidas y nivel alcanzado. */
static void DrawEndScreen(const Game *g)
{
...
/* Convierte la posicion del mouse en una casilla del tablero. */
static bool CellFromMouse(const Map *m, Vector2 mouse, int *r, int *c)
{
...
/* Detecta si el click fue cerca de un borde de casilla.
   Si fue cerca del borde derecho, es muro vertical.
   Si fue cerca del borde inferior, es muro horizontal. */
static bool WallFromMouse(const Map *m, Vector2 mouse, int *r, int *c, int *dir)
{
    int col, row;
    float localX, localY;
...
/* Inicializa el editor de mapas con un mapa 9x9 vacio. */
static void InitEditor(Editor *e)
{
    ClearMap(&e->map, 9, 9);
    e->tool = TOOL_PAC;
...
/* Guarda un mensaje temporal del editor. */
static void EditorMessage(Editor *e, const char *text)
{
...
/* Actualiza el editor:
   cambia herramienta, coloca elementos con mouse, cambia tamaño y guarda archivo. */
static void UpdateEditor(Editor *e)
{
    int r, c, dir;
...
/* Dibuja el editor de mapas y sus instrucciones. */
static void DrawEditor(const Editor *e)
{
    int i;
    ...
    DrawText("C/X: columnas", SIDE_X, 336, 17, LIGHTGRAY);
